...
async def dbstats_cmd(client, message):
	...
	await edit_or_reply(message, f"<code>→ </code> <b>online for</b> <code>{uptime}</code>" +
					...
					f"\n<code> → </code> <b>{msg_count}</b> msgs logged (+{sep(DRIVER.counter['messages'])} new | <i>{DRIVER.counter.per_second('messages'):.1f} /s</i> | <b>{msg_size}</b>)" +
					f"\n<code> → </code> <b>{service_count}</b> events tracked (+{sep(DRIVER.counter['service'])} new | <i>{DRIVER.counter.per_second('service'):.1f} /s</i> | <b>{service_size}</b>)" +
					f"\n<code> → </code> <b>{deletions_count}</b> deletions saved (+{sep(DRIVER.counter['deletions'])} new | <i>{DRIVER.counter.per_second('deletions'):.1f} /s</i> | <b>{deletions_size}</b>)" +
					f"\n<code> → </code> <b>{members_count}</b> members updated (+{sep(DRIVER.counter['members'])} new | <i>{DRIVER.counter.per_second('members'):.1f} /s</i> | <b>{members_size}</b>)" +
					f"\n<code> → </code> <b>{user_count}</b> users met (+{sep(DRIVER.counter['users'])} new | <i>{DRIVER.counter.per_second('users'):.1f} /s</i> | <b>{user_size}</b>)" +
					f"\n<code> → </code> <b>{chat_count}</b> chats visited (+{sep(DRIVER.counter['chats'])} new | <i>{DRIVER.counter.per_second('chats'):.1f} /s</i> | <b>{chat_size}</b>)" +
