...
HELP.add_help(["freq", "frequent"], "find frequent words in messages",
				...
				"Extra parameters for the db query can be given with `-q`.",
				args="[-r <n>] [-min <n>] [-g <group>] [-u <user>] [-q <{q}>] [n]", public=True)
...
async def frequency_cmd(client, message):
	...
	where = "**everywhere**"
	extra = f"`{message.command['query']}`" if "query" in message.command else ""
	if group:
